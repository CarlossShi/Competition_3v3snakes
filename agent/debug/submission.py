...
def my_controller(observation_list, action_space_list, is_act_continuous):
    ...
    if ctrl_agent_index == 0:
        ...
        for x, y in beans_positions:
            board_with_beans[x, y] = -3
        sns.heatmap(board_with_beans, annot=True, fmt='d', cbar=None)
        plt.savefig('images_init_board/' + time. strftime('%Y%m%d-%H%M%S') + '_init_board', bbox_inches='tight')
        plt.clf()
        time.sleep(0.2)
    ...
    if ctrl_agent_index == 0:
        sns.heatmap(bd.board, annot=True, fmt='d', cbar=None)
        plt.savefig('images_board/' + time.strftime('%Y%m%d-%H%M%S') + '_board', bbox_inches='tight')
        plt.clf()
